  ...
  weeks: [
    ...
    },
    {
      ...
      steps: [
        ...
        { type: "choices", id: "w2_recharge_minus", multi: true, allowOther: true, title: "나를 방전시키는 것",
          options: ["사람 많은 자리","갈등/눈치","멀티태스킹","의미 없는 반복","수면 부족","과한 정보/알림","평가/비교","연속된 급한 마감","계획이 틀어질 때","혼자 너무 오래"] },
        { type: "choices", id: "w2_work_style", multi: true, allowOther: true, title: "나는 일을 이렇게 처리해요 (나다운 방식)",
          options: ["한 번에 몰아서","조금씩 꾸준히","마감 직전 폭발","계획 먼저","일단 시작하고 고치며","혼자 깊이 파고","사람들과 의논하며","큰 그림부터","디테일부터","마음 내킬 때 몰입","정해진 순서/루틴대로","마감이 있어야"] },
        { type: "choices", id: "w2_env_setup", multi: true, allowOther: true, title: "내가 잘 되려면 이런 환경이 필요해요",
          options: ["조용한 공간","적당한 소음(카페 등)","혼자","옆에 누가 함께","마감/데드라인","자유로운 재량","뚜렷한 목표","작게 쪼갠 할 일","방해 차단(알림 끄기)","몸을 먼저 움직이고","아침 시간 확보","마음이 편한 상태"] },
        ...
        { type: "promptForge", id: "w2_ai", title: "AI와 함께 곱씹기 · 프롬프트 만들기",
          ...
          system: "너는 따뜻하지만 솔직한 자기탐구 코치야. 아래 [내 기록]은 내가 나를 관찰하며 적은 '작동방식' 기록이야. 점수나 진단이 아니라, 내가 '맞아, 이게 나야' 하고 곱씹을 수 있게 구체적으로 비춰줘. 아래 번호 구조로 써줘.\n\n§1 나를 움직이는 엔진 — 진짜 스위치가 뭔지. 어떤 조건·상황에서 내가 켜지는지.\n§2 내가 꺼지는 조건 — 절대 안 하게 되거나 금방 흩어지는 상황의 공통 패턴.\n§3 나도 몰랐던 작동 패턴 2~3개 — 기록을 가로질러 보이는, 내가 미처 의식 못 한 패턴. 모순·긴장(예: 자유를 원하면서 마감이 있어야 움직이는)도 솔직히 짚어줘.\n§4 나를 잘 되게 세팅하는 법 — 시간·공간·사람·일의 형태를 어떻게 맞추면 좋을지, 당장 실행 가능한 형태로.\n§5 추천 — (a) 지금 만들어볼 작은 습관 2~3개, (b) 우선순위를 무엇 중심으로 두면 좋을지 + 그 이유.\n§6 마지막으로, 나에게 건네는 한 문단 — 따뜻하지만 솔직하게. 단정짓지 말고 곱씹을 여지를 남겨줘.",
          collect: [
            ...
            { label: "머리가 가장 잘 도는 시간대", from: "w2_energy_peak" },
            { label: "나를 충전시키는 것", from: "w2_recharge_plus" },
            { label: "나를 방전시키는 것", from: "w2_recharge_minus" },
            { label: "일을 처리하는 나다운 방식", from: "w2_work_style" },
            { label: "잘 되려면 필요한 환경", from: "w2_env_setup" },
            { label: "나를 관통하는 가치", from: "w1_journey" },
            { label: "내가 적은 정리(나는 이렇게 작동한다)", from: "w2_summary" }
          ] },
